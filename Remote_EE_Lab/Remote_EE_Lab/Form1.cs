...
namespace Remote_EE_Lab
{
    ...
    private void Form1_Load(Object sender, EventArgs e) { }
        ...
        private void button2_Click_1(object sender, EventArgs e)
        {
            ...
            new System.IO.StreamReader("C:/Users/Gytau_000/Documents/GitHub/RLS-master/RLSinC#/Remote_EE_Lab/Remote_EE_Lab/Z80BubbleSort_2C1.hex");
            Byte highAddr;
            Byte lowAddr;
            Byte data;
            Byte temp;
            ...
            while ((line = file.ReadLine()) != null)
            {
                ...
                for (int i = 0; i < lolk.Length; i = i + 2)
                {
                    if (lolk[i] == (':'))
                    {
                        dataNum = lolk[i + 1].ToString() + lolk[i + 2].ToString();
                        toBytes = StringToByteArray(dataNum);
                        csByteIndex = toBytes[0];
                        csByteIndex = csByteIndex + 8;
                       
                        if (csByteIndex == IntelHexCSum(lolk[i].ToString()))// do checksum if it passes start reading in and sending to arduino
                        {
                            cont = true;
                            MessageBox.Show("WE DID IT WOO");
                            //send the number of data bytes
                        }
                        else {
                            cont = false;
                            MessageBox.Show("Invalid file, checksum doesn't match");
                            break;
                        }
                    ...
                    else if (i >= 10 && i < csByteIndex && cont == true)
                    {
                        dataNum = lolk[i + 1].ToString() + lolk[i + 2].ToString();
                        toBytes = StringToByteArray(dataNum);
                        data = toBytes[0];
        ...
        Byte IntelHexCSum(String InHEX)
        {
            ...
            {
                bytes.Add(Convert.ToByte("&H" + InHEX.Substring(i, 2)));
            }
